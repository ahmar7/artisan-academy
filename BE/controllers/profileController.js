...
exports.updatetProfile = catchAsyncErrors(async (req, res, next) => {
    ...
    const { profile } = req.body;
      if (file) {
        ...
        res.status(200).json({ message: 'Profile updated successfully', updatedDocument });
      } else  {
        console.log(profile)
        const updatedDocument = await userModel.findByIdAndUpdate(id, { profile:profile }, { new: true });
        res.status(200).json({ message: 'Profile updated successfully', updatedDocument });
      }
    } catch (error) {
      console.error(error);
